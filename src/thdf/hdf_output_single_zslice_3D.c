							  ...
							  int N_incl, int N_azimuth, int N_frequencies)
{
	...
	if (file < 0)
	{
		if (rank == 0) fprintf(stderr, "[single] Cannot create: %s\n", path);
		return -1;
	}
	if (rank == 0)
	{
		printf("[single] Created file: %s\n", path);
		fflush(stdout);
	}
	...
	if (rank == 0)
	{
		printf("[single] Created group: /radiation_field\n");
		fflush(stdout);
	}
	...
	if (rank == 0)
	{
		printf("[single] Obtained datatype: float32\n");
		fflush(stdout);
	}
	...
	if (rank == 0)
	{
		printf("[single] Created dataset creation property list\n");
		fflush(stdout);
	}

	{
		...
		if (rank == 0)
		{
			printf("[single] Set chunking for 6D datasets\n");
			fflush(stdout);
		}
	}

	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (dset < 0)
			{
				if (rank == 0) fprintf(stderr, "[single] Error dataset %s\n", names6[i]);
				rc = -1;
				goto cleanup;
			}
			H5Dclose(dset);
		...
		if (rank == 0)
		{
			printf("[single] Created 6D datasets\n");
			fflush(stdout);
		}
	}
	...
	if (rank == 0)
	{
		printf("[single] Created 5D datasets (normalization multipliers)\n");
		fflush(stdout);
	}
...
sync:
	...
	{
		if (rank == 0) fprintf(stderr, "[single] H5Fflush failed\n");
		rc = -1;
	}
...
cleanup:
	...
	if (nspace >= 0) H5Sclose(nspace);
	if (dcpl >= 0) H5Pclose(dcpl);
	if (dcpl_n >= 0) H5Pclose(dcpl_n);
	if (f32 >= 0) H5Tclose(f32);
	if (f64 >= 0) H5Tclose(f64);
							   ...
							   int N_azimuth, int N_frequencies)
{
	...
	if (h->group_id < 0) goto fail;

	h->datatype_f32_id = H5Tcopy(THDF_get_hdf_float32_datatype());
	if (h->datatype_f32_id < 0) goto fail;
	...
	if (h->datatype_f64_id < 0) goto fail;
...
fail:
	if (h->dataset_id_I >= 0) H5Dclose(h->dataset_id_I);
	if (h->dataset_id_Q >= 0) H5Dclose(h->dataset_id_Q);
	if (h->dataset_id_U >= 0) H5Dclose(h->dataset_id_U);
	if (h->dataset_id_V >= 0) H5Dclose(h->dataset_id_V);
	if (h->dataset_norm_multiplier_I >= 0) H5Dclose(h->dataset_norm_multiplier_I);
	if (h->dataset_norm_multiplier_QI >= 0) H5Dclose(h->dataset_norm_multiplier_QI);
	if (h->dataset_norm_multiplier_UI >= 0) H5Dclose(h->dataset_norm_multiplier_UI);
	if (h->dataset_norm_multiplier_VI >= 0) H5Dclose(h->dataset_norm_multiplier_VI);
	if (h->datatype_f32_id >= 0) H5Tclose(h->datatype_f32_id);
	if (h->datatype_f64_id >= 0) H5Tclose(h->datatype_f64_id);
	if (h->group_id >= 0) H5Gclose(h->group_id);
	free(h);
	return NULL;
}
...
THDF_write_zslab_block_single(THDF_zslab_handler_t *handler, const THDF_3D_field_t *field, int local_start_x,
							  ...
							  int N_incl, int N_azimuth, int N_frequencies)
{
	if (!handler || !handler->is_open) return -1;
...
#ifndef NDEBUG
	{
		...
		if (gc)
		{
			...
			if (!r) fprintf(stderr, "[single] collective fallback 0x%x\n", gc);
		}
	}
#endif
