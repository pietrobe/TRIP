...
extern "C"
{
	 ...
	 * The input data is expected to be in a 1D array with the following memory layout: [x, y, z, incl, azimuth,
	 * frequencies] The output data is reshuffled to match the HDF5 dataset layout: [x, y, z, incl, azimuth, frequencies]
	 * but stored in a 1D array in row-major order. The normalization is applied if the normalized_output flag is set to
	 * true, in this case the function also computes the normalization multipliers for each Stokes parameter and fills the
	 * provided buffers. The stride parameters indicate the memory layout of the input data, allowing for flexible data
	 * arrangements.
	 *
	 * @param field
	 * @param N_local_x
