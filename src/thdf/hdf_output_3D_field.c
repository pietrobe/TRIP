        ...
        for (int incl = 0; incl < N_incl; incl++) {
          for (int az = 0; az < N_azimuth; az++) {
            ...
            for (int f = 0; f < N_frequencies; f++) {
              ...
              const double QI = read_stokes_input_value(stokes_IQUI, input_is_float32, in_index_QI) / II * 100.0;
              const double UI = read_stokes_input_value(stokes_IQUI, input_is_float32, in_index_UI) / II * 100.0;
              const double VI = read_stokes_input_value(stokes_IQUI, input_is_float32, in_index_VI) / II * 100.0;
...
/* ---- verifica che la collective I/O non sia degradata ---- */
#ifndef NDEBUG
  {
    ...
    if (global_cause != 0) {
      ...
      if (rank == 0)
        fprintf(stderr, "WARN: collective I/O fallback cause=0x%x\n", global_cause);
    }
  }
#endif
