...
{ //
  //
	...
	H5E_BEGIN_TRY
	{
		output_dset->group_id = H5Gopen2(file, "/emergent_field", H5P_DEFAULT);
	}
	H5E_END_TRY;
...
{
	if (!output_field) return;

	if (output_field->stokes_I) free(output_field->stokes_I);
	if (output_field->stokes_QI) free(output_field->stokes_QI);
	if (output_field->stokes_UI) free(output_field->stokes_UI);
	if (output_field->stokes_VI) free(output_field->stokes_VI);
...
{
	if (!angular_grid) return;
	if (angular_grid->inclination_angles) free(angular_grid->inclination_angles);
	if (angular_grid->azimuthal_angles) free(angular_grid->azimuthal_angles);
	if (angular_grid->inclinations_indices) free(angular_grid->inclinations_indices);
	if (angular_grid->azimuthal_indices) free(angular_grid->azimuthal_indices);
	angular_grid->inclination_angles   = NULL;
	angular_grid->azimuthal_angles	   = NULL;
	angular_grid->inclinations_indices = NULL;
	angular_grid->azimuthal_indices	   = NULL;
...
{
	if (!frequencies_grid) return;
	if (frequencies_grid->frequencies) free(frequencies_grid->frequencies);
	frequencies_grid->frequencies	= NULL;
	frequencies_grid->N_frequencies = 0;
}
