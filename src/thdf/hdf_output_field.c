...
THDF_create_field_handler_mpi(hid_t file,           //
  ...
  H5E_BEGIN_TRY { output_dset->group_id = H5Gopen2(file, "/output_field", H5P_DEFAULT); }
  H5E_END_TRY;
  // If group doesn't exist, create it (collective operation)
  if (output_dset->group_id < 0) {
    output_dset->group_id = H5Gcreate2(file, "/output_field", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (output_dset->group_id < 0) {
  ...
  output_dset->dataset_id_I   = H5Dcreate2(output_dset->group_id, "output_field_data_I", output_dset->datatype_id,
                                           output_dset->dataspace_id_I, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  output_dset->dataspace_id_Q = H5Screate_simple(5, dims, NULL);
  output_dset->dataset_id_Q   = H5Dcreate2(output_dset->group_id, "output_field_data_Q", output_dset->datatype_id,
                                           output_dset->dataspace_id_Q, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  output_dset->dataspace_id_U = H5Screate_simple(5, dims, NULL);
  output_dset->dataset_id_U   = H5Dcreate2(output_dset->group_id, "output_field_data_U", output_dset->datatype_id,
                                           output_dset->dataspace_id_U, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  output_dset->dataspace_id_V = H5Screate_simple(5, dims, NULL);
  output_dset->dataset_id_V   = H5Dcreate2(output_dset->group_id, "output_field_data_V", output_dset->datatype_id,
...
THDF_read_field_from_hdf5(hid_t     file,           //
  ...
  hid_t group_id = H5Gopen2(file, "/output_field", H5P_DEFAULT);
  if (group_id < 0) {
    fprintf(stderr, "Error opening output_field group\n");
    return output_field;
  }

  hid_t dataset_id_I = H5Dopen2(group_id, "output_field_data_I", H5P_DEFAULT);
  hid_t dataset_id_Q = H5Dopen2(group_id, "output_field_data_Q", H5P_DEFAULT);
  hid_t dataset_id_U = H5Dopen2(group_id, "output_field_data_U", H5P_DEFAULT);
  hid_t dataset_id_V = H5Dopen2(group_id, "output_field_data_V", H5P_DEFAULT);
...
THDF_write_frequencies_grid_to_hdf5(hid_t                          file,                //
  ...
  hid_t freq_group = H5Gcreate2(file, "/output_frequencies_grid", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (freq_group < 0) {
...
THDF_read_frequencies_grid_from_hdf5(hid_t file) {
  ...
  hid_t freq_group = H5Gopen2(file, "/output_frequencies_grid", H5P_DEFAULT);
  if (freq_group < 0) {
