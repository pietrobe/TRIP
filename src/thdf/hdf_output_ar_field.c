...
THDF_create_ard_field_handler_mpi(const hid_t  file_id,            //
  ...
  *output_dset                        = THDF_make_empty_ard_field_handler();
  ...
  H5E_BEGIN_TRY { output_dset->group_id = H5Gopen2(file_id, "/output_ar_field", H5P_DEFAULT); }
  H5E_END_TRY;

  if (output_dset->group_id < 0) {
    // Group doesn't exist, create it
    output_dset->group_id = H5Gcreate2(file_id, "/output_ar_field", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (output_dset->group_id < 0) {
      fprintf(stderr, "Error creating output_ar_field group\n");
      output_dset->is_open = 0;
      free(output_dset);
      return NULL;
    }
  ...
  H5E_BEGIN_TRY { group_I = H5Gopen2(output_dset->group_id, "out_ard_beams_I", H5P_DEFAULT); }
  H5E_END_TRY;
  if (group_I < 0) {
    group_I = H5Gcreate2(output_dset->group_id, "out_ard_beams_I", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }

  H5E_BEGIN_TRY { group_Q = H5Gopen2(output_dset->group_id, "out_ard_beams_Q", H5P_DEFAULT); }
  H5E_END_TRY;
  if (group_Q < 0) {
    group_Q = H5Gcreate2(output_dset->group_id, "out_ard_beams_Q", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }

  H5E_BEGIN_TRY { group_U = H5Gopen2(output_dset->group_id, "out_ard_beams_U", H5P_DEFAULT); }
  H5E_END_TRY;
  if (group_U < 0) {
    group_U = H5Gcreate2(output_dset->group_id, "out_ard_beams_U", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }

  H5E_BEGIN_TRY { group_V = H5Gopen2(output_dset->group_id, "out_ard_beams_V", H5P_DEFAULT); }
  H5E_END_TRY;
  if (group_V < 0) {
    group_V = H5Gcreate2(output_dset->group_id, "out_ard_beams_V", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
...
THDF_write_ard_directions(hid_t                        file_id,           //
  ...
  hid_t dir_group = H5Gcreate2(file_id, "/arbitrary_directions", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (dir_group < 0) {
