...
#define CLOSE_OUTPUT_AR_FIELD(d)                                   \
	do                                                             \
	{                                                              \
		if ((d).dataset_id_I >= 0) H5Dclose((d).dataset_id_I);     \
		if ((d).dataset_id_Q >= 0) H5Dclose((d).dataset_id_Q);     \
		if ((d).dataset_id_U >= 0) H5Dclose((d).dataset_id_U);     \
		if ((d).dataset_id_V >= 0) H5Dclose((d).dataset_id_V);     \
		if ((d).datatype_id >= 0) H5Tclose((d).datatype_id);       \
		if ((d).dataspace_id_I >= 0) H5Sclose((d).dataspace_id_I); \
		if ((d).dataspace_id_Q >= 0) H5Sclose((d).dataspace_id_Q); \
		if ((d).dataspace_id_U >= 0) H5Sclose((d).dataspace_id_U); \
		if ((d).dataspace_id_V >= 0) H5Sclose((d).dataspace_id_V); \
		if ((d).group_id >= 0) H5Gclose((d).group_id);             \
		(d).is_open = 0;                                           \
	} while (0)
...
{ //
	...
	*output_dset						= THDF_make_empty_ard_field_handler();
	...
	H5E_BEGIN_TRY
	{
		output_dset->group_id = H5Gopen2(file_id, "/emergent_beams", H5P_DEFAULT);
	}
	H5E_END_TRY;
	...
	H5E_BEGIN_TRY
	{
		group_I = H5Gopen2(output_dset->group_id, "beams_I", H5P_DEFAULT);
	}
	H5E_END_TRY;
	if (group_I < 0)
	{
	...
	H5E_BEGIN_TRY
	{
		group_Q = H5Gopen2(output_dset->group_id, "beams_QI_pc", H5P_DEFAULT);
	}
	H5E_END_TRY;
	if (group_Q < 0)
	{
	...
	H5E_BEGIN_TRY
	{
		group_U = H5Gopen2(output_dset->group_id, "beams_UI_pc", H5P_DEFAULT);
	}
	H5E_END_TRY;
	if (group_U < 0)
	{
	...
	H5E_BEGIN_TRY
	{
		group_V = H5Gopen2(output_dset->group_id, "beams_VI_pc", H5P_DEFAULT);
	}
	H5E_END_TRY;
	if (group_V < 0)
	{
	...
	if (group_I < 0 || group_Q < 0 || group_U < 0 || group_V < 0)
	{
		...
		if (group_I >= 0) H5Gclose(group_I);
		if (group_Q >= 0) H5Gclose(group_Q);
		if (group_U >= 0) H5Gclose(group_U);
		if (group_V >= 0) H5Gclose(group_V);
