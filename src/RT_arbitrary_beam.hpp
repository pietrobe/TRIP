...
void compute_arbitrary_beam(RT_solver& rt_solver,
...
std::string format_arbitrary_beams_info(const std::vector<BeamDirection>& beams);

// Process all arbitrary beams and return the number of beams processed
int process_arbitrary_beams(const std::vector<BeamDirection>& beams,
                            RT_solver& rt_solver,
                            std::shared_ptr<RT_problem>& rt_problem_ptr,
                            const std::string& output_file,
