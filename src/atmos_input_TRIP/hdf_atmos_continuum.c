...
THDF_create_continuum_4D_matrices(hid_t file, int N_x, int N_y, int N_z, int N_frequencies) {
  ...
  if (create_or_open_u16_4d_dataset(cont_group, "c_scat_opacity_sigma_c", dims) < 0 ||
      ...
      create_or_open_u16_4d_dataset(cont_group, "c_tot_opacity_K_c", dims) < 0) {
    H5Gclose(cont_group);
    return -1;
  }
...
create_hdf_continuum_compound_type(const int N_frequencies) {
  ...
  H5Tinsert(continuum_type, "min_c_tot_opacity_K_c", HOFFSET(THDF_continuum_t, min_c_tot_opacity_K_c), H5T_NATIVE_DOUBLE);
  H5Tinsert(continuum_type, "max_Vm_c_tot_opacity_K_c", HOFFSET(THDF_continuum_t, max_Vm_c_tot_opacity_K_c),
            H5T_NATIVE_DOUBLE);
...
make_continuum_hdf5_mpi(hid_t file,                                      //
...

  {
    ...
    H5E_BEGIN_TRY {
      dset = H5Dcreate2(cont_group, "continuum_data", continuum_type, dspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    }
    H5E_END_TRY;

    if (dset < 0) {
      H5E_BEGIN_TRY { dset = H5Dopen2(cont_group, "continuum_data", H5P_DEFAULT); }
      H5E_END_TRY;
      if (dset < 0) {
...
normalize_continuum_data(const double *sigma_c, const double *epsilon_c, const double *K_c, int N_frequencies,
  ...
  double min_K_c       = K_c[0];
  double max_K_c       = K_c[0];
  ...
  for (int i = 0; i < N_frequencies; i++) {
    ...
    min_K_c       = K_c[i] < min_K_c ? K_c[i] : min_K_c;
    max_K_c       = K_c[i] > max_K_c ? K_c[i] : max_K_c;
  }
  ...
  cont.min_c_therm_emissivity_epsilon_c    = min_epsilon_c;
  cont.max_Vm_c_therm_emissivity_epsilon_c = max_epsilon_c - min_epsilon_c;
  cont.min_c_tot_opacity_K_c               = min_K_c;
  cont.max_Vm_c_tot_opacity_K_c            = max_K_c - min_K_c;

  for (int i = 0; i < N_frequencies; i++) {

    const double normed_sigma_c   = (sigma_c[i] - min_sigma_c) / cont.max_Vm_c_scat_opacity_sigma_c;
    const double normed_epsilon_c = (epsilon_c[i] - min_epsilon_c) / cont.max_Vm_c_therm_emissivity_epsilon_c;
    const double normed_K_c       = (K_c[i] - min_K_c) / cont.max_Vm_c_tot_opacity_K_c;
    ...
    norm_K_c[i]       = (uint16_t)((normed_K_c < 0.0 ? 0.0 : normed_K_c) * UINT16_MAX);
  }
...
denormalize_continuum_data(const THDF_continuum_t cont, const uint16_t *norm_sigma_c, const uint16_t *norm_epsilon_c,
  ...
  for (int i = 0; i < N_frequencies; i++) {
    ...
    epsilon_c[i] = cont.min_c_therm_emissivity_epsilon_c +         //
                   ((double)norm_epsilon_c[i] * inv_UINT16_MAX) *  //
                       cont.max_Vm_c_therm_emissivity_epsilon_c;   //

    K_c[i] = cont.min_c_tot_opacity_K_c +              //
             ((double)norm_K_c[i] * inv_UINT16_MAX) *  //
                 cont.max_Vm_c_tot_opacity_K_c;
  }
...
write_normalized_continuum_to_hdf5(hid_t     file,                                                          //
  ...
  /* Write norm_sigma_c, norm_epsilon_c, norm_K_c to the appropriate slice of the 4D datasets */
  ...
  /* Write c_tot_opacity_K_c dataset */
  H5E_BEGIN_TRY { dset = H5Dopen2(cont_group, "c_tot_opacity_K_c", H5P_DEFAULT); }
  H5E_END_TRY;
  if (dset >= 0) {
    dspace = H5Dget_space(dset);
    H5Sselect_hyperslab(dspace, H5S_SELECT_SET, start, NULL, count, NULL);
    mspace       = H5Screate_simple(4, mem_dims_4d, NULL);
    write_status = H5Dwrite(dset, H5T_NATIVE_UINT16, mspace, dspace, H5P_DEFAULT, norm_K_c);
    H5Sclose(mspace);
    H5Sclose(dspace);
    H5Dclose(dset);
    if (write_status < 0) {
      fprintf(stderr, "Error writing c_tot_opacity_K_c\n");
      H5Gclose(cont_group);
      return -1;
    }
  }

  /* Write cont_to_write to continuum_data dataset at (ix, iy, iz) */
  H5E_BEGIN_TRY { dset = H5Dopen2(cont_group, "continuum_data", H5P_DEFAULT); }
  H5E_END_TRY;
  if (dset < 0) {
    fprintf(stderr, "Error opening continuum_data dataset\n");
    H5Gclose(cont_group);
    return -1;
  }

  {
    ...
    dspace = H5Dget_space(dset);
    if (dspace < 0) {
      fprintf(stderr, "Error getting dataspace for continuum_data\n");
      free(cont_block);
      H5Dclose(dset);
      H5Gclose(cont_group);
      return -1;
    ...
    if (mspace < 0) {
      fprintf(stderr, "Error creating memory dataspace for continuum_data\n");
      free(cont_block);
      H5Sclose(dspace);
      H5Dclose(dset);
      H5Gclose(cont_group);
    ...
    if (mem_type < 0) {
      fprintf(stderr, "Error creating memory datatype for continuum_data\n");
      free(cont_block);
      H5Sclose(mspace);
      H5Sclose(dspace);
      H5Dclose(dset);
    ...
    if (write_status < 0) {
      fprintf(stderr, "Error writing continuum_data\n");
      H5Gclose(cont_group);
      return -1;
    }
  }
...
read_continuum_from_hdf5(hid_t     file,                                           //
...

  {
    ...
    if (H5Sselect_hyperslab(dspace, H5S_SELECT_SET, start_3d, NULL, count_3d, NULL) < 0) {
      fprintf(stderr, "Error selecting hyperslab for continuum_data\n");
      H5Sclose(dspace);
      H5Dclose(dset);
      H5Gclose(cont_group);
      return -1;
    ...
    if (H5Dread(dset, mem_type, mspace, dspace, H5P_DEFAULT, cont_to_fill) < 0) {
      fprintf(stderr, "Error reading continuum_data\n");
      H5Tclose(mem_type);
      H5Sclose(mspace);
      H5Sclose(dspace);
      H5Dclose(dset);
    ...
    norm_K_c       = (uint16_t *)malloc(vec_size * sizeof(uint16_t));

    if (norm_sigma_c == NULL || norm_epsilon_c == NULL || norm_K_c == NULL) {
...

    {
      ...
      const int f2 =
          read_u16_4d_dataset_slice(cont_group,  //
                                    "c_therm_emissivity_epsilon_c", start_4d, count_4d, mem_dims_4d, norm_epsilon_c);

      const int f3 = read_u16_4d_dataset_slice(cont_group,  //
                                               "c_tot_opacity_K_c", start_4d, count_4d, mem_dims_4d, norm_K_c);
    ...
    for (size_t cell = 0; cell < block_size; cell++) {
      ...
      denormalize_continuum_data(cont_to_fill[cell],                                                 //
                                 &norm_sigma_c[offset], &norm_epsilon_c[offset], &norm_K_c[offset],  //
                                 N_frequencies,                                                      //
                                 &sigma_c[offset], &epsilon_c[offset], &K_c[offset]);                //
    }
...
continuum_main_demo(int argc, char **argv) {
  ...
  u_int16_t *norm_K_c       = (uint16_t *)malloc(N_frequencies * sizeof(uint16_t));

  for (int i = 0; i < N_frequencies; i++) {
    sigma_c[i]   = 1.0e-2 + 1.0e-4 * i;
    epsilon_c[i] = 1.0e-3 + 1.0e-5 * i;
    K_c[i]       = 1.0e-1 + 1.0e-3 * i;
  }

  THDF_continuum_t cont =
      normalize_continuum_data(sigma_c, epsilon_c, K_c, N_frequencies, norm_sigma_c, norm_epsilon_c, norm_K_c);
