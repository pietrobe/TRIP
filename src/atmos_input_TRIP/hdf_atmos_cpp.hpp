...
namespace hdf_atmos_cpp {
  ...
  inline int
  read_geometry(hid_t file, int &N_x, int &N_y, int &N_z, std::vector<double> &heights) {
    hid_t group = H5Gopen2(file, "/geometry", H5P_DEFAULT);
    if (group < 0) {
      return -1;
    }

    if (H5LTread_dataset_int(group, "N_ij", &N_x) < 0 ||
        H5LTread_dataset_int(group, "N_height", &N_z) < 0) {
      H5Gclose(group);
      return -1;
    }
    N_y = N_x;
  ...
  read_file(const std::string &filename, int &N_x, int &N_y, int &N_z, std::vector<double> &heights, int &N_frequencies,
    ...
    try {
      if (read_geometry(file, N_x, N_y, N_z, heights) < 0) {
        H5Fclose(file);
