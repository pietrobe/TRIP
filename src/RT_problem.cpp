...
void RT_problem::read_3D(const char* filename_pmd, const char* filename_cul, const char* filename_qel, const char* filename_llp, const char* filename_back)
{
   ...
   MPI_CHECK(MPI_File_seek(fh, TWOLEVEL_HEADER1, MPI_SEEK_CUR));
	...
	MPI_CHECK(MPI_File_read_all(fh, &Ll2_,   1, MPI_INT,    MPI_STATUS_IGNORE));
	MPI_CHECK(MPI_File_read_all(fh, &Lu2_,   1, MPI_INT,    MPI_STATUS_IGNORE));
...
void RT_problem::read_3D(const char* filename){
   ...
   mass_ = atomic_buffer[0];
	...
	// if (mpi_rank_ == 0) std::cout << "WARNING: hardcoding Qel_ computation with no xi (NOT GOOD for tests)!" << std::endl;
...
std::vector<double> RT_problem::read_single_node(MPI_File fh, const int i, const int j, const int k){
    ...
    // Could be global
	const int L_LIMIT = (Ll2_+1)*(Ll2_+1);
	const int U_LIMIT = (Lu2_+1)*(Lu2_+1);
	const int NJKQ = 9;
	...
	double entry, atomic_density, rho00l, Nl, Cul;
	 ...
	 // recover populations 
	 // 9
	 Nl = atomic_density * sqrt(Ll2_ + 1) * rho00l;
	 output.push_back(Nl);	
...
void RT_problem::set_up_atom(){
	...
	{
		Jl2_vec_.push_back(i);
	}

	for (int i = std::abs(Lu2_ - S2_); i < Lu2_ + S2_ + 1; i = i + 2)
	{
		Ju2_vec_.push_back(i);
	}
...
// 	     	// if (n_nu == 48)
// 	     	// {	     		
// 			//    if (space_grid_->local_to_global_coordinate(0, i) == 2 and 
// 			//    	 space_grid_->local_to_global_coordinate(1, j) == 2 and
// 			//    	 space_grid_->local_to_global_coordinate(2, k) == 50)
// 			//    {	   	
// 			// 	   std::cout << block_eta[b]  << std::endl; 
    ...
    {             	
        ...
        {        	        	        				
			...
			{				
				...
				for (int Mu2 = - Lu2_; Mu2 < Lu2_ + 1; Mu2 += 2)
				{
					for (int Ml2 = - Ll2_; Ml2 < Ll2_ + 1; Ml2 += 2)
					{										
						if (std::abs(Mu2 - Ml2) <= 2) 
						{
							const int q2 = Ml2 - Mu2;
...
void RT_problem::set_up(){
   ...
   double tmp_const, tmp_const2, tmp_const3;
	...
	// const for k_L
	tmp_const = c_ * c_* (Lu2_ + 1) * Aul_ / (8 * PI * nu_0_ * nu_0_ * (Ll2_ + 1));

	// const for Wien 
	tmp_const2 = 2 * h_ * nu_0_ * nu_0_ * nu_0_ / (c_ * c_);
	
	// const for D1
	tmp_const3 = (Lu2_ * Lu2_ + 2 * Lu2_ - 3) / ( 3 * (Lu2_ * Lu2_ + 2 * Lu2_ - 7));
...
RT_problem::set_3D_decomposition_BLC(const int mpi_rank, const int mpi_size,const int N_x, const int N_y, const int N_z)
{
	...
	if (mpi_rank == 0)
	{
		if (verbose_ == 0) std::cout << "Starting domain decomposition for MPI size = " << mpi_size << " and grid size N_x=" << N_x << ", N_y=" << N_y
				  << ", N_z=" << N_z << std::endl;
