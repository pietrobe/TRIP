...
int main(int argc, char *argv[]) {
...

  {
    ...
    const bool output_overwrite_prevention =
        true; // if true the application stops (with an MPI_Abort) if the output
    ...
    if (use_CRD)
      emissivity_model_var = emissivity_model::CRD_limit;
    if (use_continuum)
      emissivity_model_var = emissivity_model::ZERO;

    // Set here the main input and output directories //////////////////////////
#if USE_CMD_LINE_OPTIONS == 1
    const std::filesystem::path main_input_dir =
        getOptionArgument(argc, argv, "--input_dir");
    const std::filesystem::path main_output_dir =
        getOptionArgument(argc, argv, "--output_dir");
#else
    //  const std::filesystem::path main_input_dir  = "../input/PORTA";
    const std::filesystem::path main_input_dir =
        "/scratch/snx3000/pietrob/Comparison-TRIP-PORTA/a63/";
    const std::filesystem::path main_output_dir =
        "/scratch/snx3000/pietrob/Comparison-TRIP-PORTA/a63/output";
#endif
    ////////////////////////////////////////////////////////////////////////////
...
// Set here the problem input file //////////////////////////
#if USE_CMD_LINE_OPTIONS == 1
    std::string input_pmd_string =
        getOptionArgument(argc, argv, "--problem_pmd_file");
    ...
    const std::string input_config_string =
        getOptionArgument(argc, argv, "--problem_input_config");

    if (not input_config_string.empty()) {

      const auto config_map = get_input_PORTA(
          main_input_dir / std::filesystem::path(input_config_string),
          mpi_rank);
...
#else
    const std::string input_pmd_string =
        std::string("AR_385_Cut_64x64_mirrorxy-CRD_I_V0-V0_fix_conv_KQ_MC.pmd");
    const std::string input_llp_string =
        std::string("AR_385_Cut_64x64_mirrorxy-CRD_I_V0-V0_fix_conv_KQ_MC.llp");

    const std::string input_cul_string =
        std::string("AR_385_Cut_64x64_mirrorxy-CRD_I_V0_fix.cul");
    const std::string input_qel_string =
        std::string("AR_385_Cut_64x64_mirrorxy-CRD_I_V0_fix.qel");
    const std::string input_back_string =
        std::string("AR_385_Cut_64x64_mirrorxy-CRD_I_V0_fix.back");
#endif
    /////////////////////////////////////////////////////////////

    // const auto input_pmd_file = std::filesystem::path(input_pmd_string);

    auto frequencies_input_path =
        main_input_dir / std::filesystem::path("frequency/96F");

    auto PORTA_input_pmd =
        main_input_dir / std::filesystem::path(input_pmd_string);

    // lambda to build the RT_problem object
    auto create_rt_problem = [&]() {
      if (input_cul_string.empty() or input_qel_string.empty() or
          input_llp_string.empty()) {
        ...
        if (mpi_rank == 0)
          std::cout
              << "INPUT reading: using PMD + CUL + QEL + LLP + BACK input files"
              << std::endl;

        // NUOVO // PMD + CUL + QEL + LLP input
        // create cul and qel input path
        auto input_cul_path =
            main_input_dir / std::filesystem::path(input_cul_string);
        auto input_qel_path =
            main_input_dir / std::filesystem::path(input_qel_string);
        auto input_llp_path =
            main_input_dir / std::filesystem::path(input_llp_string);
        auto input_back_path =
            main_input_dir / std::filesystem::path(input_back_string);
        ...
        if (mpi_rank == 0) {
          ...
          ss_a << "PMD input file:   "
               << file_name_provided(PORTA_input_pmd.string()) << std::endl;
          ss_a << "LLP input file:   "
               << file_name_provided(input_llp_path.string()) << std::endl;
          ss_a << "CUL input file:   "
               << file_name_provided(input_cul_path.string()) << std::endl;
          ss_a << "QEL input file:   "
               << file_name_provided(input_qel_path.string()) << std::endl;
          ss_a << "BACK input file:  "
               << file_name_provided(input_back_path.string()) << std::endl;
          std::cout << ss_a.str();
        }

        return std::make_shared<RT_problem>(
            PORTA_input_pmd.string().c_str(), input_cul_path.string().c_str(),
    ...
    auto rt_problem_ptr =
        create_rt_problem(); /// CALL LAMBDA to create RT_problem object
    ...
    if (rt_problem_ptr->mpi_rank_ == 0) {
      ...
      ss_a << "PetscInt size: " << petsc_index_size << " bytes; "
           << (petsc_index_size * 8) << " bits." << std::endl
           << std::endl;
    ...
    // inputs
    auto problem_input_file =
        std::filesystem::path("FAL-C/B20_V0_12T_8C_99F_1Pi4_9Pi8");
    ...
    const int N_theta = 8;
    const int N_chi = 16;
    auto rt_problem_ptr = std::make_shared<RT_problem>(
        problem_input_FAL.string(), N_theta, N_chi, use_CRD, use_B);
#endif
    ...
    if (output) {
      const std::filesystem::path output_path =
          ...
          std::filesystem::path(
              input_pmd_string + "." +
              emissivity_model_to_string_long(emissivity_model_var));
      ...
      if (rt_problem_ptr->mpi_rank_ == 0) {
        if (not std::filesystem::exists(output_path)) {
          ...
          std::cerr << "File: " << __FILE__ << " Line: " << __LINE__
                    << " MPI rank: " << rt_problem_ptr->mpi_rank_
                    << " Directory: " << output_path << " already exists"
                    << std::endl;
      ...
      output_file = (use_CRD) ? (output_path / "profiles_CRD").string()
                              : (output_path / "profiles_PRD").string();
...
#if ACC_SOLAR_3D == _ON_
      {
        if (RII_epsilon_contrib::RII_contrib_MPI_Is_Device_Handler()) {
          ...
          std::string file_name =
              (boost::format("%s/device_pool_info_rank_%d.txt") % output_path %
               mpi_rank)
                  .str();
    ...
    const auto compute_arbitrary_beam =
        [&, Nx = rt_problem_ptr->N_x_, Ny = rt_problem_ptr->N_y_](
          ...
          mu_str.erase(std::remove(mu_str.begin(), mu_str.end(), '.'),
                       mu_str.end());
          chi_str.erase(std::remove(chi_str.begin(), chi_str.end(), '.'),
                        chi_str.end());

          std::string output_file_Omega_mu =
              output_file + "_mu" + mu_str + "_chi" + chi_str;
          ...
          for (int i = 0; i < Nx; ++i) {
            for (int j = 0; j < Ny; ++j) {
              rt_problem_ptr->write_surface_point_profiles_Omega(
                  output_file_Omega_mu, i, j);
            }
          }
        };
    ...
    if (output) {
      ...
      std::vector<Real> mus_vec = {
          0.1, 0.3, 0.7, 1.0}; //// ATTENTION: arbitrary beam directions
...
#pragma message "ATTENTION: using hardcoded extra arbitrary beams"
        {
          ...
          // {0.98, 0.99, 0.997};
          const std::vector<Real> mus_extra = {0.12, 0.3,
                                               0.77}; // Test with different mu

          for (auto mux : mus_extra)
            mus_vec.push_back(mux);
        ...
        for (Real mu : mus_vec) {
          ...
          if (rt_problem_ptr->mpi_rank_ == 0) {
            std::cout << getCurrentDateTime()
                      << " - Computing arbitrary beam: mu = " << mu
                      << ", chi = " << chi << std::endl;
          }
      ...
      if (rt_problem_ptr->mpi_rank_ == 0) {
        ...
        std::cout << "Number of beams          = " << beams_counter
                  << std::endl;
        std::cout << "Time per beam (s)        = "
                  << (tock - tick) / double(beams_counter) << std::endl;
      }
    ...
    if (rt_problem_ptr->mpi_rank_ == 0) {
      ...
      ss_mem << "Total memory usage (vm_usage) = " << byte_to_GB * vm_usage
             << " GB" << std::endl;
      ss_mem << "Total memory usage (resident_set) = "
             << byte_to_GB * resident_set << " GB" << std::endl;
  ...
  //     std::string file_name =
  //         (boost::format("%s/pool_info_rank_%d.txt") % output_path %
  //         mpi_rank)
  //             .str();
