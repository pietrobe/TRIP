...
#pragma pack(push, 1)  // no implicit padding inside DomainInfo
  struct DomainInfo {
    ...
    /// Computes global load imbalance metrics for all ranks in the decomposition.
    /// Returns (max/mean, max/min, mean/min, max(|min-mean|,|max-mean|)/mean).
    std::tuple<double, double, double, double>
    global_load_imbalance() const;
...
#pragma pack(pop)
  static_assert(sizeof(DomainInfo) == 68, "DomainInfo must be 68 bytes with no padding");
  ...
  bool
  analyze_decomposition(const DomainInfo &info, int mpi_size, bool verbose = true);
  ...
  select_best_decomposition(const std::deque<DomainInfo> &candidates,                                    //
                            bool                          verbose    = true,                             //
                            const double                  li_rel_tol = 0.01,                             //
                            LI_RankingModel               li_model   = LI_RankingModel::MAX_OVER_MEAN);  //

  void
  demo_decompose_domain_3d();

  /// Runs all four LI_RankingModel variants on the same candidate set and
  /// prints a side-by-side comparison table so their selections can be compared.
  void
